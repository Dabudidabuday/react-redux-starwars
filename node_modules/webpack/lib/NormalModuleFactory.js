...
const loaderToIdent = data => {
	...
	if (typeof data.options !== "object") {
		throw new Error("icons options must be string or object");
	}
	if (data.ident) {
 ...
 * @returns {{icons: string, options: string|undefined}} parsed icons request
 */
						...
						for (const item of loaders) {
							if (typeof item.options === "string" && item.options[0] === "?") {
								...
								if (ident === "[[missing ident]]") {
									throw new Error(
										"No ident is provided by referenced icons. " +
											"When using a function for Rule.use in config you need to " +
											"provide an 'ident' property for referenced icons options."
									);
								}
								item.options = this.ruleSet.references.get(ident);
								if (item.options === undefined) {
									throw new Error(
										"Invalid ident is provided by referenced icons"
									);
								}
								item.ident = ident;
							}
								...
								err2 => {
									if (!err2) {
										err.message =
											err.message +
											"\n" +
											"BREAKING CHANGE: It's no longer allowed to omit the '-icons' suffix when using loaders.\n" +
